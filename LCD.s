...
loop1
	...
	AND R2, R2, R3
	CMP R2, #0x10
	BEQ loop1		;wait for BUSY bit to be low
	
	LDR R1, =DC		;clear DC to 0
	LDR R2, [R1]
	MOV R2, #0
	STR R2, [R1]
...
loop2
	...
	AND R2, R2, R3
	CMP R2, #0x10
	BEQ loop2			;wait for BUSY to be low
...
loop3
	...
	LDR R1, =DC		;set DC to one
	LDR R2, [R1]
	MOV R2, #0x40
	STR R2, [R1]
	
	LDR R1, =SSI0_DR_R   ;write 8-bit data to SSIO_DR_R
	STR R0, [R1]
